...
<div class="mt-10 grid gap-10 px-4 lg:px-6">
	<Card.Root>
		...
		<Card.Content>
			{#if !templateExists}
				<p class="mb-2 text-sm text-red-500">Template file not found. Please upload a template.</p>
			{/if}
			<Button href="/api/download-template" disabled={!templateExists}
		...
		>
			<Card.Content>
				...
				{#if uploadSubmitError}
					<p class="my-2 text-sm text-red-600">{uploadSubmitError}</p>
				{/if}
			</Card.Content>
	...
	<Card.Root>
		...
		<Card.Content>
			...
			<div class="mt-5 grid gap-5 [&_div>img]:w-[50%] [&_div>p]:text-xl [&_div>p]:font-bold">
