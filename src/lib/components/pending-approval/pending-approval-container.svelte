<script lang="ts">
  ...
  let { filedLeave, hrInfo } : { filedLeave: AdminFiledLeaveInfo[], hrInfo: EmployeeData} = $props()
...
<Card.Root class="w-full h-fit!">
  <Card.Header class='flex'>
    ...
    <Button variant='link' href="/admin/requests/history" >View History</Button>
  </Card.Header>
  <Card.Content class="grid gap-3">
    {#if filedLeave.length > 0}
       {#each filedLeave as i}
          <PendingApprovalCard  filedInfo={i} {hrInfo} />
       {/each}
    {:else}
